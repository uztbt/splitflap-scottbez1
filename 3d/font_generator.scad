...
render_alignment_marks =
    false; // Whether to render markings to help with alignment/registration (e.g. for screen printing)
...
assert(!(layout_mode != MODE_DOUBLE_SIDED && render_alignment_marks),
       "Alignment marks are only supported with duplex double-sided layout mode");
...
{
    x_pos = layout_mode == MODE_SIDE_BY_SIDE ? (flap_width * 2 + spacing_x * 2) * col : (flap_width + spacing_x) * col;
    y_pos = layout_mode == MODE_FULL_FONT    ? (flap_height * 2 + flap_gap + spacing_y) * row
            : layout_mode == MODE_PRINT_FONT ? (flap_height * 2 + flap_gap + spacing_y) * row
...
module configured_flap(index, flap, front_letter, back_letter)
{
    ...
    {
        flap_with_letters(flap_color, letter_color, 0, flap_gap, flap = flap, front_letter = front_letter,
                          back_letter = back_letter, bleed = bleed);
        translate([ 0, -flap_pin_width - flap_gap, 0 ])
        {
            rotate([ 180, 0, 0 ])
            {
                flap_with_letters(flap_color, letter_color, index - 1, flap_gap, flap = flap,
                                  front_letter = back_letter, back_letter = front_letter, bleed = bleed);
            }
        }
    }

    if (layout_mode == MODE_PRINT_FONT)
    {
        if (index % 2 == 0)
        {
            flap_with_letters(flap_color, letter_color, index, flap_gap, flap = flap, front_letter = front_letter,
                              back_letter = back_letter, bleed = bleed, cut_tabs = true);
            translate([ flap_width, -flap_gap*2, 0 ]) mirror([ 1, 0, 0 ]) translate([ 0, -flap_pin_width, 0 ])
            {
                rotate([ 180, 0, 0 ])
                {
                    flap_with_letters(flap_color, letter_color, index + 1, flap_gap, flap = flap,
                                      front_letter = front_letter, back_letter = back_letter, bleed = bleed,
                                      cut_tabs = true);
                }
            }
        }
        ...
        {
            translate([ flap_width, 0, 0 ]) mirror([ 1, 0, 0 ]) flap_with_letters(
                flap_color, letter_color, index - 1, flap_gap, flap = flap, front_letter = back_letter,
                back_letter = front_letter, bleed = bleed, cut_tabs = true);
            translate([ 0, -flap_pin_width -flap_gap*2, 0 ])
            {
                rotate([ 180, 0, 0 ])
                {
                    flap_with_letters(flap_color, letter_color, index, flap_gap, flap = flap,
                                      front_letter = back_letter, back_letter = front_letter, bleed = bleed,
                                      cut_tabs = true);
                }
            }
    ...
    if (layout_mode == MODE_SIDE_BY_SIDE)
    {
        ...
        translate([ 2 * flap_width + spacing_x, 0 ])
        {
            rotate([ 0, 180, 0 ])
            {
                flap_with_letters(flap_color, letter_color, index, flap_gap, flap = flap, front_letter = back_letter,
                                  back_letter = front_letter, bleed = bleed);
            }
        }
    }
}
...
module alignment_marks()
{
    ...
    // Holes in each corner, intended for alignment pins when building a flap tray, which register the flap tray to the
    // printing platen for repeatability
    color([ 1, 0, 0 ])
    {
        linear_extrude(height = 5)
        {
            ...
            translate(
                [ cols * flap_width + (cols - 1) * spacing_x, flap_height + alignment_offset - flap_pin_width / 2 ])
            {
                circle(d = alignment_diameter, $fn = 30);
            }
            translate([
                cols * flap_width + (cols - 1) * spacing_x,
                -(flap_height + spacing_y) * (visible_rows - 1) - alignment_offset - flap_pin_width / 2
            ])
            {
                circle(d = alignment_diameter, $fn = 30);
            }
    ...
    // Additional alignment holes in each corner, intended as registration marks for aligning the screen/image to the
    // flap tray during initial setup
    color([ 0, 0, 1 ])
    {
        linear_extrude(height = 5)
        {
            ...
            translate([
                cols * flap_width + (cols - 1) * spacing_x - secondary_alignment_offset,
                flap_height + alignment_offset - flap_pin_width / 2
            ])
            {
                circle(d = alignment_diameter, $fn = 30);
            }
...
projection_renderer(render_index = render_index, render_etch = render_etch, kerf_width = kerf_width, panel_height = 0,
                    panel_horizontal = 0, panel_vertical = 0)
{
    _flip()
    {
